...
import com.callcenter.common.schedule.base.BaseMultiTask;
 ...
 * Time: 下午11:45
 */
public final class SyncTaskSchedule extends BaseMultiTask<SyncTask> {

    private static final Log log = LogFactory.getLog(SyncTaskSchedule.class);
    @Resource
     ...
     * @param ownSign           BASE
     * @param taskItemNum,     任务项个数，同配置文件中一致
     * @param taskItemList     本台server分到的任务项
     * @param eachFetchDataNum 每次查询的数据量  所有参数提供在select时使用，需要自己处理
     * @return 任务集合
     * @throws Exception
     */
    @Override
    public List<SyncTask> selectTasks(String taskParameter, String ownSign,
                                      int taskItemNum, List<TaskItemDefine> taskItemList,
                                      int eachFetchDataNum) throws Exception {
        System.out.println("SyncTaskSchedule-->selectTasks("+taskParameter+","+ownSign+","+taskItemNum+","+taskItemList+","+eachFetchDataNum+")");
        ...
        for (SyncTask queryTask : queryTaskList) {
            ...
            if (CommonConstants.MAX_FAIL_COUNT > queryTask.getFailCount()) {
                /*List<TaskItemDefine> itemList  = taskItemList;*/
               /* if(isMyTask(queryTask.getId(),itemList,eachFetchDataNum))*/{
                    //更新同步任务状态为同步中
                    if (syncTaskDao.updateByKey(initSyncTask4Update(queryTask, CommonConstants.SYNC_STATUS_3,
                            CommonConstants.SYNC_STATUS_2)) > 0) {
                ...
                queryTaskList.remove(queryTask);
            }
        }
     ...
     */
    @Override
    public boolean execute(SyncTask[] tasks, String ownSign) throws Exception {
        for (SyncTask task : tasks) {
            log.info("执行任务的个数为" + tasks.length);
            SyncTask st4Update;
            try {
                //根据处理结果返回 做相应处理
                if (handleSyncTaskService.parseAndCallService(task)) {
                    ...
                    if (CommonConstants.SYNC_TYPE_CYCLE.equals(task.getSyncType())) {
                        st4Update = initSyncTask4Update(task, CommonConstants.SYNC_STATUS_2, CommonConstants.SYNC_STATUS_3);
                        syncTaskDao.updateByKey(st4Update);
                    } else {
                        st4Update = initSyncTask4Update(task, CommonConstants.SYNC_STATUS_4, CommonConstants.SYNC_STATUS_3);
                        syncTaskDao.updateByKey(st4Update);
                    }
                } else {
                    st4Update = initSyncTask4Update(task, CommonConstants.SYNC_STATUS_2, CommonConstants.SYNC_STATUS_3);
    ...
    class TaskCheckThread implements Runnable {
        ...
        private CopyOnWriteArrayList<SyncTask> toRunTaskList;
